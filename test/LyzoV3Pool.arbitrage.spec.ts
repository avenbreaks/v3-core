...
import { LyzoV3PoolSwapTest } from '../typechain/LyzoV3PoolSwapTest'
...
describe('LyzoV3Pool arbitrage tests', () => {
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          const arbTestFixture = async ([wallet, arbitrageur]: Wallet[]) => {
            ...
            const testerFactory = await ethers.getContractFactory('LyzoV3PoolSwapTest')
            const tester = (await testerFactory.deploy()) as LyzoV3PoolSwapTest
